...
class QRec(object):
    def __init__(self, config):
        ...
        '''
        获得self.relation，也就是social网络于item网络，对应text文件中的二元组组成了三元组，第三列加了个1
        '''
        ...
        if config.contains('item'):
            self.itemConfig = OptionConf(self.config['item.setup'])
            self.itemRelation = FileIO.loadItemRelationship(config, self.config['item'])
            ...
            for train, test in DataSplit.crossValidation(self.trainingData, k, binarized=binarized):
                ...
                if self.config.contains('social'):
                    recommender = self.config[
                                      'model.name'] + "(self.config,train,test,self.relation, self.itemRelation, fold)"
                else:
