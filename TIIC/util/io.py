...
class FileIO(object):
    ...
    @staticmethod
    def loadItemRelationship(conf, filePath):
        itemConfig = OptionConf(conf['item.setup'])
        relation = []
        print('loading item data...')
        with open(filePath) as f:
            relations = f.readlines()
            # ignore the headline
        if itemConfig.contains('-header'):
            relations = relations[1:]
        # order of the columns
        order = itemConfig['-columns'].strip().split()
        for lineNo, line in enumerate(relations):
            items = split(' |,|\t', line.strip())
            if len(order) < 2:
                print('The social file is not in a correct format. Error: Line num %d' % lineNo)
                exit(-1)
            itemId1 = items[int(order[0])]
            itemId2 = items[int(order[1])]
            if len(order) < 3:
                weight = 1
            else:
                weight = float(items[int(order[2])])
            relation.append([itemId1, itemId2, weight])
        return relation
